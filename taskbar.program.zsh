...
for (( i=1;  i<=${#ids}; i++ ))
{
	...
	[[ -z $icon ]] && icon=$default 
	#enquanto o loop rodar para atribuição de janela por janela, salvar o resultado na variável $labels, uma linha por janela aberta.
	labels="${labels}\n$indice ${ids[$i]} $workspace $icon $program $title" 
}
#retira linhas em branco de $labels e escreve o resultado em /tmp/taskbar ${bd}.
grep '[[:digit:]]' <<< "$(echo $labels |sort -n -k1)" > $bd 
...
declare -x contadorold=20
#caso o arquivo exista em /tmp/taskbar ele vai criá-lo novamente zerado.
>| $bd
#inicia o programa.
start
